...
#[cfg(feature = "python")]
pub(crate) fn apply_node_to_targets<'py>(
...
#[cfg(feature = "python")]
pub(crate) fn apply_node_to_image<'py>(
    node: RandomImageNode,
    p: Dist,
    array: &'py PyAny,
    inplace: Option<bool>,
...
#[cfg(feature = "python")]
pub(crate) fn parse_p_dist(p: Option<&PyAny>) -> PyResult<Dist> {
...
pub(crate) fn extract_node(item: &PyAny) -> PyResult<RandomImageNode> {
    // Identity transform
    if let Ok(obj) = item.extract::<PyRef<super::choice::PyIdentity>>() {
        return Ok(maybe_wrap(RandomImageNode::Identity, obj.p.clone()));
    }

    // Choice transform
    if let Ok(obj) = item.extract::<PyRef<super::choice::PyChoice>>() {
        let node = obj.to_node(item.py())?;
        return Ok(maybe_wrap(node, obj.p.clone()));
    }
